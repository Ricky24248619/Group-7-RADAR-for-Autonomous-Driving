...
on this machine, so training from scratch was never in scope.
    ...
    python scripts/truckscenes_fcos3d_infer.py \
        --dataroot <man-truckscenes> --config <fcos3d config .py> \
        --checkpoint <fcos3d .pth> --start 0 --end 15 --out results.json

Requires a dedicated venv, not the tutorial one — see experiment-log/0006 for
the exact package set (numpy<2, torch==2.1.0+cpu, mmcv/mmdet/mmdet3d,
truckscenes-devkit) and the version pitfalls that make this venv fussy.
...
# TruckScenes has two extra classes (animal, traffic_sign) that a
# nuScenes-trained model can never predict — expected, not a gap.
NUSC_TO_TRUCKSCENES = {
    "car": "car",
    "truck": "truck",
    "trailer": "trailer",
...
def boxes_cam_to_global(bboxes_3d, scores, labels,
    ...
    nuscenes-devkit's Box class — plain numpy/pyquaternion only. Copied
    rather than re-derived so the camera-to-global geometry is provably the
    same math OpenMMLab uses to score FCOS3D on its native dataset.
    """
